...
public class EventLayout : MonoBehaviour
{
	...
	private void get_data()
	{
		...
		foreach (var t in events)
		{
			...
			if (/*int.Parse(date[0]) >= startDate.month && int.Parse(date[0]) <= endDate.month && 
			    int.Parse(date[1]) >= startDate.day && int.Parse(date[1]) < endDate.day &&
			    int.Parse(date[2]) >= startDate.year && int.Parse(date[2]) <= endDate.year)*/
				tday >= startDate && tday < endDate)
			{
